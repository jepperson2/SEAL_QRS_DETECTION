...
//CLIENT SIDE
void HE::setup(EncryptionParameters parms) {
    parms.validate();
	...
	if(debug){
        ...
        cout << left_margin << "parms.poly_modulus: " << parms.poly_modulus().to_string() << endl;
		cout << left_margin << "parms.coeff_modulus: " << parms.coeff_modulus().to_string() << endl;
		cout << left_margin << "parms.plain_modulus: " << parms.plain_modulus().to_string() << endl;
		cout << left_margin << "parms.decomposition_bit_count: " << parms.decomposition_bit_count() << endl;
		cout << left_margin << "parms.noise_standard_deviation: " << parms.noise_standard_deviation() << endl;
		cout << left_margin << "parms.noise_max_deviation: " << parms.noise_max_deviation() << endl;
...
void HE::EFF_RCMP(vector<mkt> &k1_bit, vector<mkt> &k2_bit){
	/ *      Purpose: Efficient Ripple Comparator
                                 (equality and lower than, for unsigned numbers)
                Inputs: Bit k1[0],..., bit k1[nbits - 1] and 
                                bit k2[0],..., bit k2[nbits - 1]
                                Note that nbits can be different for k1 and k2 but these 
